...
export const isMessageInNarrow = (
  ...
  if (
    isStreamNarrow(narrow) && message.display_recipient === narrow[0].operand
  ) {
    return true;
  }
  ...
  if (isPrivateNarrow(narrow) || isGroupNarrow(narrow)) {
    ...
    const normalizedNarrow = [...narrow[0].operand.split(','), selfEmail]
      .sort()
      .join(',');

    return normalizedRecipients === selfEmail ||
