...
export const registerAppActivity = (auth: Auth, hasFocus = true) => {
  if (
    differenceInSeconds(new Date(), lastAppActivity) >
    APP_ACTIVITY_UPDATE_FREQUENCY
  ) {
...
export const registerUserInputActivity = (auth: Auth) => {
  if (
    differenceInSeconds(new Date(), lastInputActivity) >
    USER_INPUT_ACTIVITY_UPDATE_FREQUENCY
  ) {
