...
describe('getFilteredEmojiList', () => {
  ...
  test('return zulip extra emojis in order of which starts with and then which includes query', () => {
    const list = getFilteredEmojiList(
      'lip',
      {},
      {
        zulip: { source_url: '/user_avatars/2/emoji/zulip.png' },
        lip: { source_url: '/user_avatars/2/emoji/lip.png' },
      },
    );
    expect(list).toEqual(['lips', 'lipstick', 'lip', 'zulip']);
  });
});
